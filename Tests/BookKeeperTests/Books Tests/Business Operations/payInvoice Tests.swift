...
extension BooksTests {
    ...
    func testPayInvoiceInsufficientBalanceErrors() throws {
        ...
        XCTAssertEqual(books.suppliers.first?.value.payables.balance, 55_555)
        ...
        XCTAssertThrowsError(
            try books.payInvoice(amount: 50_000, to: Supplier.sample.id)
        ) { error in
            XCTAssertEqual(error as? AccountError,
    ...
    func testPayInvoiceNoErrors() {
        ...
        XCTAssertEqual(books.suppliers.count, 1)
        XCTAssertEqual(books.suppliers.first?.value.id, Supplier.sample.id)
        XCTAssertEqual(books.suppliers.first?.value.payables.balance, 5_555)

        XCTAssertEqual(books.ledger.count, 2)
        XCTAssertEqual(books.ledger[.cash]?.balance, 50_000)
        XCTAssertEqual(books.ledger[.payables]?.balance, 5_555)
    }
